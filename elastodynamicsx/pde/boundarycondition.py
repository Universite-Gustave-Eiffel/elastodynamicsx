...
class BoundaryCondition():
    ...
    Clamp    : bc = BoundaryCondition(V, facet_tags, 'Clamp', (1,2))         <- imposes u=0 on boundaries n°1 and 2
    ...
    Dashpot  : bc = BoundaryCondition(V, facet_tags, 'Dashpot', 1, z)          <- (scalar) imposes sigma(u).n=z*v on boundary n°1, with v=du/dt. Usually z=rho*c
    Dashpot  : bc = BoundaryCondition(V, facet_tags, 'Dashpot', 1, (z_N, z_T)) <- (vector) imposes sigma(u).n=z_N*v_N+z_T*v_T on boundary n°1, with v=du/dt, v_N=(v.n)n, v_T=v-v_N
